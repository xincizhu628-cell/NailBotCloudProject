(() => {
 ...
 async function renderCoupons(){list.textContent='Loading coupons...';try{const {coupons}=await load();list.innerHTML=coupons.map(c=>{const url=safeUrl(c.coupon_url),expired=c.expiry_date&&String(c.expiry_date).toLowerCase()!=='always'&&Date.parse(c.expiry_date)<=Date.now();return `<article class="shop-coupon-item"><h3 data-no-translate>${esc(c.coupon_name)}</h3><p>${esc(window.CouponPicker.summary(c))}</p><p>有效期：${esc(window.CouponPicker.formatDate(c.start_date))} — ${esc(window.CouponPicker.formatDate(c.expiry_date))}</p>${expired?'<p>已过期</p>':url?`<a href="${esc(url)}">Get coupon / 获取优惠券</a>`:'<p>领取方式待公布</p>'}</article>`;}).join('')||'<p>暂无商城优惠券</p>';}catch(e){list.textContent=e.message;}}
