...
function createTemplateRepository(pool){
 ...
 async function image(c,owner,data){const id='asset_'+crypto.randomUUID().replace(/-/g,'');await c.query("INSERT INTO assets(asset_id,owner_user_id,asset_type,mime_type,url,base64_data) VALUES ($1,$2,'template-image',$3,$4,$5)",[id,owner,data.mime||null,data.url||(data.base64?'/api/template-image?id='+encodeURIComponent(id):''),data.base64||'']);return id;}
