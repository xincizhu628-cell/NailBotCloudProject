...
function createOrderManagementService(pool) {
  async function detail(id, client=pool) {
    ...
    const items=(await client.query('SELECT i.*,p.product_name FROM order_items i LEFT JOIN products p ON p.product_id=i.product_id WHERE i.order_id=$1 ORDER BY i.order_item_id',[id])).rows;
