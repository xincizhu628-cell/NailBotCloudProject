...
test('code routes require separate tokens and disallow manufacturer writes',async()=> {
  let response;
  const handler=createOrderCodeRoutes({codes:{list:async()=>[]},env:{ADMIN_CODE_API_TOKEN:'admin',MANUFACTURER_CODES_API_TOKEN:'factory'},readJson:async()=>({}),sendJson:(res,status,body)=>{response={status,body};}});
  const res={setHeader(){}};
...
test('actual paid-order flow stores all codes and reuses order for payment retry',async()=> {
  ...
  try {
    ...
    const context={crypto,hasPostgresRuntime:()=>true,ensurePgAdminRuntimeSchema:async()=>{},pgPool:pool,orderCodes:codes,cleanPgText:x=>String(x||''),pgNumber:x=>Number(x||0),resolveOrderUserId:async()=> 'u',ensurePrintServiceProduct:async()=>null,notifyManufacturerOrder:async()=>({status:'not_configured',error:'',response:''})};
