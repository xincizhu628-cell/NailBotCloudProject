...
async function main() {
  ...
  try {
    if (process.argv.includes('--apply-coupon-events')) {
      await client.query('BEGIN');
      try {await client.query(fs.readFileSync(path.join(root,'database/migrations/20260922_coupon_events.sql'),'utf8'));await client.query('COMMIT');console.log('Coupon-event migration applied');}catch(error){await client.query('ROLLBACK');throw error;}
    } else if (process.argv.includes('--apply-coupons')) {
      await client.query('BEGIN');
